...
RTTI_BEGIN_CLASS_NO_DEFAULT_CONSTRUCTOR(nap::VbanReceiverApp)
...
namespace nap 
{
	 ...
	 */
	bool VbanReceiverApp::init(utility::ErrorState& error)
	{		
		// Retrieve services
		...
		if (!mResourceManager->loadFile("vbanreceiver.json", error))
			return false;
		...
		// Fetch audio entity
		mVbanEntity = scene->findEntity("vbanReceiverEntity");
	 ...
	 */
	void VbanReceiverApp::update(double deltaTime)
	{
	 ...
	 */
	void VbanReceiverApp::windowMessageReceived(WindowEventPtr windowEvent)
	{
	 ...
	 */
	void VbanReceiverApp::inputMessageReceived(InputEventPtr inputEvent)
	{
